...
#define WITHFALSEDETECTION true

#define THRESHOLDSTART 1.5//1.5
...
void* real_work_thread(void *arg)
{
    ...
    fileHandlers.dataINFD.open(DATAINFDFILENAME);
    std::vector <double> smoothed_envelopeFD;
    if(fileHandlers.dataINFD.is_open()){
        while(std::getline(fileHandlers.dataINFD,line)){
            smoothed_envelopeFD.push_back(std::stod(line));
        }
    }
    else{
        std::cout<<"Error opening file FD"<<std::endl;
        pthread_exit(0);
    }
    fileHandlers.dataINFD.close();
    ...
    //set false detection threshold
    double thresholdFD;
    thresholdFD = calcMean(smoothed_envelopeFD)+(x*calcSTD(smoothed_envelopeFD));

    
    //false detection channel buffer window for inhibiting detections
    unsigned int bufferWindow = BUFFERWINDOW*3; //x*3 samples is x ms on either side at 3kHz.
    ...
    while (i<smoothed_envelopeT2.size()){ //loop through all elements of channel
        if(smoothed_envelopeT2[i] > thresholdT2){
            ...
            //check if it's a false detection
            bool falseDetection = false;
            if(WITHFALSEDETECTION){
                //check bufferwindow size before and after the current detection to see
                //if a cortical channel has increased ripple band power which would
                //indicate noise
                for(unsigned int loopy=i-bufferWindow; loopy<bufferWindow+i; ++loopy){
                    if(smoothed_envelopeFD[loopy] > thresholdFD){
                        falseDetection = true;
                        break;
                    }
                }
